...
5. **Out-of-domain / Chit-chat handling**: If the user asks general questions ("How are you?", "What's the weather like?", or even general beauty advice without a buying intent), the only actions encoded by the agent are `Ask` (clarifying constraints) or `Recommend`. Thus, it struggles to provide a natural response and might force a recommendation. The simple fix is to add a dedicated "Chit Chat" node (with an appropriate persona/name) and let the orchestrator route non-shopping queries there.
